...
> [!IMPORTANT]
> ### 🎉 v0.1.3+ 最新重大功能：自动按键映射 & 托盘运行
>
> - **⌨️ 自动按键映射（Key Mappings）**：现已支持在手机端发送文字后，自动在 PC 端模拟追加特定的按键序列（例如回车、Tab 等，或在 PC 端 `设置 → 按键映射配置` 中自定义的如 `ctrl+alt+a` 等任意快捷键）。手机端支持 WebSocket 实时热同步更新列表、LocalStorage 本地选择缓存以及失效安全降级保护。详情请见 [USER_GUIDE.md](file:///home/coding/workspace/PhoneMic/USER_GUIDE.md#38-设置追加按键映射)。
> - **📥 Windows 托盘后台运行**：主窗口点击关闭或最小化时会自动隐藏至系统托盘后台运行，不再占用任务栏，右键托盘菜单选择“退出”即可安全彻底退出进程并释放端口。详情请见 [USER_GUIDE.md](file:///home/coding/workspace/PhoneMic/USER_GUIDE.md#6-出问题了怎么办)。
>
> 建议直接阅读完整的 [用户使用指南 (USER_GUIDE.md)](USER_GUIDE.md) 获取最新特性的详细配置方法。
...
- ✅ **自动按键映射 (v0.1.3+)** – 发送后自动追加按键模拟（如回车、Tab 或自定义组合键），PC端管理且手机端实时热更新同步，具备失效安全降级机制
- ✅ **托盘后台运行 (v0.1.3+)** – 窗口关闭/最小化时自动隐藏至托盘，不占任务栏，后台保持连接与输入服务
- ✅ **实时预览** – 电脑悬浮窗实时显示输入内容，无焦点不干扰
- ✅ **自动发送** – 语音识别结束后自动上屏，流畅高效
- ✅ **语音命令** – 自定义文字触发模拟按键或运行外部程序
- ✅ **安全网络适应 (v0.1.3+)** – 前端 WebSocket 连接协议自适应安全网络 (WSS)
